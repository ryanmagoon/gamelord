...
function storeWithPhase(phase: ArtworkSyncPhase): ArtworkSyncStore {
  ...
  if (phase !== null) {
    store.setPhase(mockGame.id, phase);
  }
  return store;
}
...
describe("GameCard", () => {
  ...
  describe("artworkSyncPhase", () => {
    it("shows TV static with sync pulse during hashing phase", () => {
      ...
      const { container } = render(
        <GameCard artworkSyncStore={storeWithPhase("hashing")} game={mockGame} onPlay={onPlay} />,
      );
    ...
    it("shows TV static with sync pulse during querying phase", () => {
      const onPlay = vi.fn();
      const { container } = render(
        <GameCard artworkSyncStore={storeWithPhase("querying")} game={mockGame} onPlay={onPlay} />,
      );
    ...
    it("shows TV static with sync pulse during downloading phase", () => {
      ...
      const { container } = render(
        <GameCard
          artworkSyncStore={storeWithPhase("downloading")}
          game={mockGame}
          onPlay={onPlay}
        />,
      );
    ...
    it('shows "Artwork not found" label and game title during not-found phase', () => {
      const onPlay = vi.fn();
      render(
        <GameCard artworkSyncStore={storeWithPhase("not-found")} game={mockGame} onPlay={onPlay} />,
      );
  ...
  describe("favorite heart", () => {
    ...
    it("shows filled heart with fill-current class when game is favorited", () => {
      ...
      const { container } = render(
        <GameCard game={favGame} onPlay={onPlay} onToggleFavorite={onToggleFavorite} />,
      );
