 ...
 * direction using a weighted alignment + distance score that balances nearby targets with row and column alignment.
 */
...
export function findNextFocusable(
  ...
  candidates: Array<FocusableRect>,
  direction: Direction,
): FocusableRect | null {
 ...
 *
 * The alignment term provides a small preference for candidates in the same
 * row (for horizontal moves) or same column (for vertical moves) are
 * preferred without skipping nearby staggered rows.
 */
 ...
 */
function computeOverlap(a: FocusableRect, b: FocusableRect, isHorizontal: boolean): number {
 ...
 * on both axes, normalized by dividing by 100
...
function edgeDistance(
  ...
  const primaryGap = isHorizontal ? xGap : yGap;
  const perpendicularGap = isHorizontal ? yGap : xGap;
  return Math.hypot(primaryGap, perpendicularGap) / 100;
}
