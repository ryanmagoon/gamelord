...
describe("findNextFocusable", () => {
  it("does not skip staggered keyboard rows for a distant aligned action", () => {
    const keys = [
      { id: "a", x: 0, y: 0, width: 40, height: 40 },
      { id: "z", x: 44, y: 44, width: 40, height: 40 },
      { id: "space", x: 0, y: 132, width: 88, height: 40 },
    ];
    expect(findNextFocusable(keys[0], keys, "down")?.id).toBe("z");
  });
  // 3x3 grid layout:
  //   A  B  C
  //   D  E  F
  //   G  H  I
  const grid: Array<FocusableRect> = [
    gridRect("A", 0, 0),
    gridRect("B", 1, 0),
    gridRect("C", 2, 0),
    gridRect("D", 0, 1),
  ...
  describe("non-uniform grid (different sizes)", () => {
    it("handles cards with different aspect ratios", () => {
      const items: Array<FocusableRect> = [
