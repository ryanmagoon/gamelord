...
export const GameWindow: React.FC = () => {
  ...
  useEffect(() => {
    ...
    api.on("game:video-frame", (raw: unknown) => {
      ...
      const canvas = canvasRef.current;
      if (!canvas || !bootReadyRef.current) {
        return;
      }
          ...
          const renderLoop = (timestamp: number) => {
            if (lastFrameTimeRef.current > 0) {
              ...
              if (delta > 0) {
                ...
                fpsEmaRef.current =
                  fpsEmaRef.current === 0 ? instantFps : 0.9 * fpsEmaRef.current + 0.1 * instantFps;
            ...
            if (
              useSharedBuffersRef.current &&
              controlViewRef.current &&
  ...
  const isPointInElement = useCallback(
    (x: number, y: number, ref: React.RefObject<HTMLDivElement | null>) => {
  ...
  const isCursorOverControls = useCallback(() => {
    ...
    return (
      isPointInElement(pos.x, pos.y, topBarRef) || isPointInElement(pos.x, pos.y, bottomBarRef)
    );
  }, [isPointInElement]);
  ...
  const handleMouseMove = useCallback(
    (event: React.MouseEvent) => {
      if (mode !== "native" || isPoweringOn || isPoweringOff) {
        return;
      }
      ...
      if (last.x === clientX && last.y === clientY) {
        return;
      }
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-black/80 shadow-lg">
                ...
                <div className="absolute bottom-full left-0 mb-2 bg-black/95 rounded-lg shadow-lg py-1 min-w-[100px]">
                  {SPEED_OPTIONS.map((speed) => (
                    <button
                      ...
                      onClick={() => {
                        playSfx("click");
                        void handleSetSpeed(speed, true);
                        setShowSpeedMenu(false);
                      }}
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              size="sm"
              onClick={() => {
                playSfx(isMuted ? "toggleOn" : "toggleOff");
                setIsMuted((m) => !m);
              }}
              className="text-white hover:bg-white/20 hover:text-white"
            >
            ...
            <div className="relative">
              ...
              {showSettingsMenu && (
                <div className="absolute bottom-full right-0 mb-2 bg-black/95 rounded-lg shadow-lg py-1 min-w-[160px]">
                  ...
                  <button
                    onClick={() => {
                      playSfx(sfxPreferences.enabled ? "toggleOff" : "toggleOn");
                      setSfxEnabled(!sfxPreferences.enabled);
                    }}
                    className="w-full flex items-center justify-between px-4 py-2 text-sm text-white/80 hover:bg-white/10 transition-colors"
                  >
                    <span>UI Sounds</span>
                    <span
                      className={`ml-3 text-xs font-medium ${sfxPreferences.enabled ? "text-green-400" : "text-white/40"}`}
                    >
                  ...
                  {process.env.NODE_ENV === "development" && (
                    ...
                    >
                      <span>Annotate</span>
                      <span
                        className={`ml-3 text-xs font-medium ${showAgentation ? "text-green-400" : "text-white/40"}`}
                      >
