...
export const GameWindow: React.FC = () => {
  ...
  useEffect(() => {
    ...
    api.on('game:av-info', (avInfoRaw: unknown) => {
      const avInfo = avInfoRaw as AVInfo
      const canvas = canvasRef.current
      if (canvas) {
    ...
    api.on('game:video-frame', (frameDataRaw: unknown) => {
      const frameData = frameDataRaw as VideoFrame
