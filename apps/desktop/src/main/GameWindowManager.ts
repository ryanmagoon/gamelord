...
export class GameWindowManager extends EventEmitter {
  ...
  createNativeGameWindow(
    ...
    const fallbackRatio =
      getDisplayType(game.systemId) === "crt" ? 4 / 3 : baseWidth / baseHeight;
    const aspectRatio =
      coreAspectRatio && coreAspectRatio > 0 ? coreAspectRatio : fallbackRatio;
    ...
    if (process.env.ELECTRON_RENDERER_URL) {
      ...
      gameWindow.loadFile(path.join(__dirname, "../renderer/game-window.html"));
    }
