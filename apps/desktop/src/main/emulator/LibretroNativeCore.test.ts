...
describe("LibretroNativeCore", () => {
  ...
  describe("constructor", () => {
    it("creates save state, SRAM, and BIOS directories", () => {
      ...
      expect(mkdirSync).toHaveBeenCalledWith(path.join("/tmp/test-userdata", "savestates"), {
        recursive: true,
      });
      expect(mkdirSync).toHaveBeenCalledWith(path.join("/tmp/test-userdata", "saves"), {
        recursive: true,
      });
      expect(mkdirSync).toHaveBeenCalledWith(path.join("/tmp/test-userdata", "BIOS"), {
        recursive: true,
      });
    });
  ...
  describe("launch", () => {
    it("throws if emulator is already running", async () => {
      ...
      await expect(
        core.launch("/roms/game.nes", { corePath: "/cores/core.dylib" }),
      ).rejects.toThrow("Emulator is already running");
    });

    it("throws if corePath is not provided", async () => {
      await expect(core.launch("/roms/game.nes")).rejects.toThrow("Core path is required");
    });
