...
- [x] **Favorites** — Heart toggle on game cards (filled when favorited, visible on hover otherwise, pop animation on toggle). Favorites filter button in toolbar, favorites sorted to top within any sort order. Toggle also available in card dropdown menu. Persisted via `library:updateGame` IPC. — [#77](https://github.com/ryanmagoon/gamelord/issues/77)
