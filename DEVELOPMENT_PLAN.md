...
- [x] **Graceful app startup** — `show: false` + `ready-to-show` on BrowserWindow, inline critical dark CSS in index.html, `#root` opacity fade-in on React mount — [#125](https://github.com/ryanmagoon/gamelord/issues/125)
...
- [ ] **Per-system controller button accuracy** — `SYSTEM_BUTTONS` and `SYSTEM_LABEL_OVERRIDES` are rough guesses for most systems. Need to verify against each libretro core's actual retropad mapping and add correct labels (e.g. PSX ×/○/□/△, Saturn A-Z, Genesis 6-button, N64 full C-button set). — [#217](https://github.com/ryanmagoon/gamelord/issues/217)
...
- [ ] **Shader playground** — Interactive parameter editor panel in the game window. Runtime uniforms for shader parameters, sliders/toggles/color pickers with live preview, save/load custom presets, A/B comparison, parameter search. Presentational component in `packages/ui/` with full Storybook coverage. — [#179](https://github.com/ryanmagoon/gamelord/issues/179)
...
- [x] **Auto-updates** — `electron-updater` + GitHub Releases for automatic update checks (on launch + every 6 hours) and silent background downloads. Non-intrusive `UpdateNotification` banner with progress bar, "Restart Now" button, and auto-dismissing errors. "Check for Updates" button in Settings About tab. Release workflow updated to `--publish always` with `GH_TOKEN` for native electron-builder publishing (DMG + zip + `latest-mac.yml` manifest). — [#185](https://github.com/ryanmagoon/gamelord/issues/185)
...
- ~~Oxlint fails in git worktrees due to config discovery walking up into the main repo's `oxlint.config.ts`, causing a WeakSet validation mismatch between two `defineConfig()` instances~~ — Fixed: lint scripts now pass `-c ../../oxlint.config.ts` explicitly. New packages must include this flag.
