...
- **Check for transitive `@types/node` pollution.** After adding a dependency, run `pnpm why @types/node` and verify the hoisted version still matches the project's pinned version (`^22` via `pnpm.overrides`). Transitive deps that pull in a newer `@types/node` can silently break `tsgo` type resolution on CI.
