...
- **Never use framework auto-setup CLIs** (`storybook add`, `npx sb init`, and similar codemod installers) in this pnpm monorepo. They can pick the wrong package manager (leaving a stray `yarn.lock` and a broken `node_modules`) and run stale codemods. Add dependencies with `pnpm` and write the configuration manually.
- **Keep `storybook` and all `@storybook/*` packages on the same minor version.** Addons import from `storybook/internal/*`, and a version skew fails with confusing "no matching export" errors at test/build time.
- **Check for duplicate React copies after lockfile re-resolution.** Any add/update that re-resolves the lockfile can split the graph into two `react`/`react-dom` versions (e.g. a nested copy under `@testing-library/react`), which breaks hooks with "invalid hook call" errors that look like product bugs. Verify with `pnpm why react react-dom` (or check for `node_modules/*/node_modules/react-dom`) and fix with `pnpm dedupe react react-dom`.
